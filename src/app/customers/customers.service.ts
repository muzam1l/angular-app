...
function matches(customer: Customer, term: string, pipe: PipeTransform) {
  return customer.first_name.toLowerCase().includes(term.toLowerCase())
    // || pipe.transform(customer.last_name.toLowerCase()).includes(term)
    || pipe.transform(customer.order_total.toLowerCase()).includes(term);
}
